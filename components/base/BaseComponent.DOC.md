...
Чтобы избежать асинхронного запроса `fetch` (который вызывает моргание интерфейса), можно задать шаблон прямо в классе:
...
- создаёт `<style id="style-ИмяКласса">` и добавляет в `<head>`;
- вставка идёт сразу **после `<title>`**, если он есть;
- если `<title>` отсутствует, BaseComponent пишет предупреждение в `console.warn`
  и добавляет стили в конец `<head>`;

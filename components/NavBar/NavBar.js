...
export class NavBar extends BaseComponent {
  ...
  _updateUserName() {
    ...
    const nameSpan = this.querySelector('.nav-bar__title span');
    if (nameSpan) {
  ...
  _setupEventListeners() {
    const btn = this.querySelector('.my-component__btn');
    if (btn) {
      btn.addEventListener('click', () => {
        this.dispatchEvent(new CustomEvent('nav-bar-action', {
          detail: {
...
<script>
  document.querySelector('nav-bar').addEventListener('nav-bar-action', (event) => {
    console.log('Событие от nav-bar!', event.detail);
